...
class myPDF extends FPDF
{
    ...
    function headerTable()
    {
        ...
        $this->Cell(35, 10, 'Kegiatan', 1, 0, 'C');
        $this->Cell(20, 10, 'Alfa', 1, 0, 'C');
    ...
    {
        $this->SetFont('Times', '', 12);
        $byr = $db->query('select * from raport_ramadhan');
        while ($data = $byr->fetch(PDO::FETCH_OBJ)) {
            $this->Cell(95, 10, $data->nama_santri, 1, 0, 'L');
            $this->Cell(23, 10, $data->tanggal, 1, 0, 'C');
            $this->Cell(20, 10, $data->waktu, 1, 0, 'C');
            $this->Cell(35, 10, $data->kegiatan, 1, 0, 'C');
            $this->Cell(20, 10, $data->alfa, 1, 0, 'C');
            $this->Cell(20, 10, $data->izin, 1, 0, 'C');
            $this->Cell(25, 10, $data->sakit, 1, 0, 'C');
            $this->Ln();
        }
    }
}
